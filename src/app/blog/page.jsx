    ...
    <div className="p-8 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Theo dõi và quản lý nội dung blog
          </p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-5 mb-8">
        <Stat title="Tổng bài viết" value={total} color="text-blue-600" />
        <Stat title="Đã xuất bản" value={published} color="text-green-600" />
        <Stat title="Bản nháp" value={draft} color="text-gray-600" />
        <Stat title="Lượt xem" value={views} color="text-purple-600" />
      </div>
      ...
      <div className="bg-white rounded-xl shadow">
        ...
        <div className="p-4 flex gap-4 border-b">
          <input
            placeholder="🔍 Tìm kiếm bài viết..."
            className="border px-4 py-2 rounded-lg flex-1 focus:outline-none focus:ring-2 focus:ring-green-500"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
          />
          ...
          <tbody>
            {currentData.map((b) => (
              ...
              >
                ...
                <td className="p-4 text-amber-600">{b.category?.name}</td>

                <td className="p-4">{b.author_name}</td>
          ...
          <div className="flex gap-2">
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
              <button
                ...
                ${page === p ? "bg-green-600 text-white" : "hover:bg-gray-100"}
                `}
              >
                {p}
              </button>
